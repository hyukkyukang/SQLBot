...
-- Create user
DROP ROLE IF EXISTS sqlbot;
CREATE USER sqlbot WITH PASSWORD 'sqlbot_pw';
...
-- Grant privileges
GRANT CONNECT ON DATABASE test_db TO sqlbot;
ALTER DATABASE test_db OWNER TO sqlbot;
GRANT insert, delete, update, select on cars to sqlbot;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO sqlbot;
...
-- Grant privileges
GRANT CONNECT ON DATABASE test_db2 TO sqlbot;
ALTER DATABASE test_db2 OWNER TO sqlbot;
GRANT insert, delete, update, select on cars to sqlbot;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO sqlbot;
